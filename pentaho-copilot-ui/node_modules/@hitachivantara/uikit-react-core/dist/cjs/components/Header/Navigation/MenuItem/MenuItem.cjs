...
const HvHeaderMenuItem = ({
  ...
  const {
    classes,
    cx
  } = MenuItem_styles.useClasses(classesProp);
  const selectionPath = React.useContext(SelectionContext.SelectionContext);
  const {
    dispatch
  } = React.useContext(FocusContext.FocusContext);
  const {
    data
  } = item;
  ...
  const label = /* @__PURE__ */ jsxRuntime.jsx(Typography.HvTypography, { component: "span", variant: isSelected ? "label" : "body", "data-text": item.label, children: item.label });
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("li", { id, className: cx(classes.root, {
    [classes.menu]: isMenu,
    ...
    hasSubLevel && currentLevel < levels && currentLevel < 2 && /* @__PURE__ */ jsxRuntime.jsx(Bar.Bar, { data, type: "menu", children: data.map((itm) => /* @__PURE__ */ jsxRuntime.jsx(HvHeaderMenuItem, { item: itm, type: "menu", onClick, levels, currentLevel: currentLevel + 1 }, itm.id)) })
  ] }, item.label);
};
